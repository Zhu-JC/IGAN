...
plot_sankey = function(result,cells_group,gene,match_list1,match_list2,p,minGenes){
  ...
  use_gene = which(rowSums(Gene_corM>0) > 100)
  ...
  for (v in 1:length(useligand)) {
    ...
    test = bitr(data, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
    ...
    recepter_name = CellChatDB[["interaction"]][["receptor"]][rank]
    recepter_name = recepter_name[!grepl(':',recepter_name)]
    recepter = strsplit(recepter_name,'_')
    recepter = sapply(recepter, function(x){
      ans = bitr(x, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
      return(ans[,2])
    })
    ...
    use_recepter = recepter_name[which(recepter2>0)]
    use_recepter = paste0(use_recepter,' ')
    R = c(R,use_recepter)
    recepter = recepter[which(recepter2>0)]
    #构建nodes
